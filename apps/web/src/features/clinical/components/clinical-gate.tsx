...
import { clinicalScreen } from '../screens';

/**
 * The screen-level permission gate.
 *
 * The same courtesy the admin console and the front office offer, for the same
 * reason: navigation already hides what a session cannot open, but a link gets
 * pasted into a handover note, bookmarked, or sent to the colleague covering the
 * clinic. The screen answers by explaining (`docs/06` §6.7) rather than by
...
export function ClinicalGate({
  ...
  const screen = clinicalScreen(screenKey);
