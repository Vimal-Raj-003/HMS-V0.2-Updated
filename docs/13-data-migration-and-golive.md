...
> the *method*, EN-036 is the *machine*. Companion docs: `09-quality-gates-and-testing.md` §12 (UAT),
...
The default answer to "can we bring everything?" is **no** — and that must be a *decision*, not an accident.
...
| **Vendor-provided export** (CSV/Excel) | vendor cooperates but restricts access | agree the exact column list and a re-extract SLA (you *will* need a second extract) | encoding damage, Excel mangling long numbers and leading zeros, truncation, silent row limits |
...
2. **Deterministic merges** (same ABHA, same verified Aadhaar hash, identical mobile+DOB+name) may auto-merge
   *before* clinical records are attached — i.e. during the master load, never after.
...
**KPIs — measure the baseline *before* cutover, or the benefit case is unprovable.**
