...
### Why not Supabase as the platform (while still supporting it as a _provider_)
...
**But nothing is thrown away:** because we depend only on _standard PostgreSQL_ + RLS, a pure-cloud tenant can be
...
| ------------------------ | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Framework                | **Next.js 15 App Router + React 19**                      | one codebase for staff workspaces, patient portal, kiosk & TV; RSC gives fast first paint on weak hospital PCs; file-based routing maps cleanly to role workspaces; huge hiring pool; deploys to Vercel _or_ a container on-prem (`output: 'standalone'`) |
...
PostHog (product analytics on _staff_ usage only, never PHI), and Cloudflare.
