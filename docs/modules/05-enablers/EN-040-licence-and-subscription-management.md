...
- Degradation state is visible to every user as a banner with a plain explanation and a "contact your administrator" action; the _reason_ shown to a clinician is never a cryptic licence error mid-procedure.
...
- Collected by event subscription and hourly rollups into `lic_usage_daily` (no PHI, no identifiers beyond counts), aggregated to `lic_usage_periods` at cycle close, and made visible to the hospital _before_ it is billed (transparency prevents disputes).
