...
**Rule of thumb for builders:** if it touches an _order, a schedule, a worklist, a report or a dose_, it belongs to **OP-008**. If it touches _pixels, DICOM objects, archives or viewers_, it belongs to **EN-008**. If it touches _message format_, it belongs to **EN-019**; _message delivery_ belongs to **EN-017**.
...
- **No parallel RIS.** If a third-party RIS remains in production (see §9), exactly one system is the _master of the order_ and the other is a subscriber; the master is declared per-tenant in configuration and cannot be ambiguous.
...
- **AI-004 radiology assist** (triage of critical findings, auto-measurement, prior comparison) plugging into the OP-008 reading worklist as a _suggestion_ layer.
