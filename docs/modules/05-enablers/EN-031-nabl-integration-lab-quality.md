...
- **TAT** is measured by OP-004 events but *governed* here: targets per test/priority (stat/urgent/routine) across segments — collection→receipt, receipt→analysis, analysis→verification, verification→report delivery — with % within target, median and 90th percentile, outlier drill-down and reasons.
