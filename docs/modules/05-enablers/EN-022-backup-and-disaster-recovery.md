...
- The tier is set per hospital (licence-linked, EN-040) and drives schedules, replica topology, alert thresholds and drill frequency. **The dashboard always shows the *measured* RPO/RTO from the last drill next to the *target*** — if they diverge, that is the finding.
...
1. Drills are **scheduled** (Platinum/Gold quarterly, Silver half-yearly, Bronze annually) with a calendar entry, a named lead, participants and a scenario from the library: *primary DB loss*, *whole-site loss*, *ransomware encryption of the primary and repo1*, *accidental mass deletion by an admin*, *object storage loss*, *PACS archive loss*, *WAN/ISP loss with on-prem survival*.
...
- Immutable copies: object-lock (S3 Object Lock / MinIO WORM) in *compliance mode* for a defined period (e.g. 14 days) so even a compromised admin credential cannot delete or encrypt them; separate credentials with write-once permissions, held outside the domain/AD used by the HMS.
