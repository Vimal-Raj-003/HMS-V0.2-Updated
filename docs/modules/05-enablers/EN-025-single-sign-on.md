...
- The result is recorded on the action's audit entry (`auth_context`), so an auditor can see *how strongly* the person was authenticated when they approved a ₹2 lakh write-off.
