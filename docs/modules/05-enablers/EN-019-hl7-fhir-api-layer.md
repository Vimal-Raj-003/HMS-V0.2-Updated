...
- Results (`ORU`/`Observation`) marked _final_ are immutable; corrections arrive as `corrected` results creating a new version with `status=corrected` and never overwrite history (mirrors OP-004 rules).
