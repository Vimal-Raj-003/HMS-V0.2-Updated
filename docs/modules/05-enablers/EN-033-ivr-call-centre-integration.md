...
4. **Authentication before any personal data**: a 4-digit OTP sent by SMS to the same number, or DOB verification (DDMM), configurable per flow. Report *content* is never read out; only status and, if permitted, "your report is ready — collect at the lab or check the portal, link sent by SMS".
...
- **No personal or clinical data is disclosed before authentication** (OTP to the calling number or DOB match). Report *content*, diagnoses and values are never read out by IVR — status only, with a portal link by SMS.
