...
3. **Identification** (any of, per configuration):
   ...
   - **ABHA QR scan-and-share** (EN-011): the patient scans the kiosk's displayed ABHA QR *from their app*, or presents their ABHA QR to the kiosk scanner → demographic + ABHA number received with a consent artefact → matched to the MPI or offered as a new registration.
...
6. **Privacy display rules**: names partially masked on shared screens, no diagnosis or clinical values ever rendered on a kiosk, report *content* never displayed (print or link only), and a privacy-filter-friendly layout (critical info in the centre, large text).
...
- **Report tasks require mobile OTP**; UHID+DOB is never sufficient to release a document. Report *content* is never rendered on screen — print or secure link only.
