...
| Consumes | `lab.micro.result` (OP-004), `nursing.line.inserted|removed`, `icu.vent.*`, `icu.line.*`, `ot.case.completed`, `nursing.wound.recorded`, `ip.admitted|transferred|discharge.completed`, `pharmacy.issued{antibiotic}`, `cssd.cycle.failed` (EN-003), `device.env.reading` (EN-042), `order.created{antibiotic}` (OP-002) | | | | |
