...
Defaults: BB technician (37): donor.*, collection, test.enter, component.prepare, inventory.*, temp, crossmatch.perform, issue.confirm, mtp.issue; BB MO/in-charge: + test.validate, crossmatch.validate, issue.emergency approval, discard, reaction.workup, reports/export, configure, integration; Ward/ICU/OT nurse (17/18/20): request.create (with doctor), transfusion.record, reaction.report; Doctors: request.create, mtp.activate, request.read; Lab quality (35): report.read; Billing: read; SoD: `crossmatch.perform` user ≠ `issue.confirm` verifier for the same unit; witness must be different user.
