...
Defaults: Phlebotomist: sample._; Technician: order.list/read, result.enter/verify (not own entries), qc.manage (runs); Pathologist: result.validate/amend, critical.notify, report._, sensitive.read; Quality manager: qc.*, master.configure (ranges w/ approval), report.read; Doctor: result.read (care team), critical.read; Front office: order.create (walk-in), report.print/deliver; Auditor: read.
