...
async function seedMastersRegistry(ctx: SeedContext): Promise<void> {
  const masters: readonly (readonly [string, string, string, string, string, number])[] = [
    ...
    [
      'code_system',
      'Code systems (ICD, LOINC, SNOMED, ATC, UCUM)',
    ...
    [
      'payer',
      'Payers (TPA, insurer, corporate, government)',
      'mdm.mdm_payers',
      'hospital',
      'insurance_executive',
      5,
...
async function seedWidgetCatalogue(ctx: SeedContext): Promise<void> {
  const widgets: readonly (readonly [string, string, string, string, string, readonly string[]])[] = [
    [
      ...
      'queue.token.current',
      'masked',
      ['opd_token', 'pharmacy_queue', 'cash_counter'],
    ],
    [
      'next_tokens',
      'Next tokens',
      'queue',
      'queue.token.upcoming',
      'masked',
      ['opd_token', 'pharmacy_queue'],
    ],
    [
      'queue_summary',
      'Queue summary',
    ...
    [
      ...
      'queue.counter',
      'masked',
      ['cash_counter', 'opd_token'],
    ],
    [
      'doctor_availability',
      'Doctor availability',
      'clinical',
      'opd.doctor.availability',
      'none',
      ['doctor_availability', 'opd_token'],
    ],
    [
      'doctor_delay_notice',
      'Doctor delay notice',
    ...
    [
      ...
      'none',
      ['opd_token', 'ward_status', 'signage_only'],
    ],
    [
      'health_tip_ticker',
      'Health tip ticker',
    ...
    [
      ...
      'Visiting hours',
      'signage',
      'display.visiting_hours',
      'none',
      ['ward_status', 'signage_only'],
    ],
