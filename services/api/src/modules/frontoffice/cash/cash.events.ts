...
/**
 * Builds an NC-001 outbox event from the registry, validating the payload.
 *
 * Same reasoning as `admin.events.ts`: `OutboxService.publish` takes the event
 * type as a plain string, so `receipt.isued` would be written happily,
 * relayed happily and consumed by nobody — the receipt would never reach the
 * patient by WhatsApp and never reach the day book, and nothing would error. `docs/09 §4` requires every event
